$(window).load(function() {
  function init() {
    var length = $('.carouselM div').length / 2;
    var min = $('.carouselM div').length;
    ...
    if (min <= 4) {
      ...
      $('.carouselM').find('div').first().addClass('ic_active');
      $('.carouselM').find('div').first().find('img').addClass('mainimg');
      
      //Divide Imgs in half and take the second half and move them to top of stack for wrap effect
      var getHalf = Math.ceil(length);
      var wrapImgs = $('.carouselM').find('div').slice(getHalf, min);
      $('.carouselM').prepend(wrapImgs);
      infinite();
    }
  }//end init
  init();
  ...
  function infinite() {
    ...
    function clickMe() {
      ...
      if(clicked.hasClass('back_btn')){
        ...
        var bkbtn = clicked.parent().find('.carouselM').find('.prev');
        clicked = bkbtn;


      }else if (clicked.hasClass('next_btn')){

        var nxbtn = $(this).parent().find('.carouselM').find('.next');
        clicked = nxbtn;
      }
